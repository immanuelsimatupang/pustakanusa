...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        $stmt = $conn->prepare("SELECT id, username, email, password, name, two_factor_enabled FROM users WHERE username = ? OR email = ?");
        ...
        if ($result->num_rows == 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                $_SESSION['login_verified'] = true; // User has successfully logged in with password
                ...
                if ($user['two_factor_enabled'] == 1) {
                    ...
                    // No 2FA required, go directly to dashboard
                    header("Location: dashboard.php");
                    exit;
                }
            } else {
